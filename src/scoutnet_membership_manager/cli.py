...
def _canary(args: argparse.Namespace) -> int:
    from scoutnet_membership_manager.jobs.canary import run_canary
...
def _drift(_args: argparse.Namespace) -> int:
    from scoutnet_membership_manager.jobs.spec_drift import run_drift_check
...
def _check_idempotency(client: object, member: str, target: int) -> int:
    ...
    from scoutnet_membership_manager.write.executor import write_status_token
...
def _verify_write(args: argparse.Namespace) -> int:  # noqa: PLR0911 - guard-heavy CLI handler
    ...
    """
    from scoutnet_membership_manager.scoutnet.client import ScoutnetError
    from scoutnet_membership_manager.write import IntendedMove, RunMode, WriteExecutor
...
def _validate_config(args: argparse.Namespace) -> int:
    ...
    from scoutnet_membership_manager.config.loader import ConfigError, load_config
...
def main(argv: list[str] | None = None) -> int:
    """Main."""
    p = argparse.ArgumentParser(prog="scoutnet_membership_manager")
