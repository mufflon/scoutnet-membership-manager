...
def _entry(no, name, tgt, tgtid, status=MoveStatus.READY):
    return MoveEntry(
        ...
        member_name=name,
        birth_year=2016,
        source_avdelning="Hajarna",
        source_troop_id=10155,
        target_avdelning=tgt,
        target_troop_id=tgtid,
        transition=TransitionKind.SAME_WEEKDAY,
        status=status,
    )
...
def test_changelist_requires_ack_for_off_cohort():
    ...
    with pytest.raises(ChangelistAckRequired):
        build_changelist(
            master, kar_name="Finn", term_label="Höst 2026", generated_at=WHEN, config_version="v1"
        )
    # with an acknowledgement it succeeds
    data = build_changelist(
        master,
        kar_name="Finn",
        term_label="Höst 2026",
        generated_at=WHEN,
        config_version="v1",
        ack_by="alex",
        ack_at=WHEN,
